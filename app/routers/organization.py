...
@router.get(
    ...
    dependencies=[
        Depends(require_permission(Permission.ORGANIZATION_PROFILE_READ))
    ],
)
...
@router.put(
    ...
    response_model=OrganizationResponse,
    dependencies=[
        Depends(require_permission(Permission.ORGANIZATION_PROFILE_UPDATE))
    ],
)
...
@router.get(
    "/settings",
    dependencies=[
        Depends(require_permission(Permission.ORGANIZATION_SETTINGS_READ))
    ],
)
...
@router.put(
    "/settings",
    dependencies=[
        Depends(require_permission(Permission.ORGANIZATION_SETTINGS_UPDATE))
    ],
)
...
@router.get(
    ...
    dependencies=[
        Depends(require_permission(Permission.ORGANIZATION_STAFF_READ))
    ],
)
...
@router.post(
    ...
    status_code=201,
    dependencies=[
        Depends(require_permission(Permission.ORGANIZATION_STAFF_MANAGE))
    ],
)
...
def update_staff(staff_id: int, payload: StaffUpdate, db: Session = Depends(get_db), context: OrganizationContext = Depends(get_organization_context)):
    ...
    if (
        "role" in update_data
        and update_data["role"] not in ASSIGNABLE_ORGANIZATION_ROLES
    ):
        raise HTTPException(status_code=400, detail="Invalid organization role")
...
@router.get(
    ...
    dependencies=[
        Depends(require_permission(Permission.ORGANIZATION_SUBSCRIPTION_READ))
    ],
)
...
@router.get(
    "/feature-flags",
    dependencies=[
        Depends(require_permission(Permission.ORGANIZATION_FEATURE_FLAGS_READ))
    ],
)
...
@router.get(
    ...
    dependencies=[
        Depends(require_permission(Permission.ORGANIZATION_AUDIT_LOGS_READ))
    ],
)
def list_audit_logs(
